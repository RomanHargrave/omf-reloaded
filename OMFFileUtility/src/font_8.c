...
void saveFont8(font8 f, FILE * pFile)
{
...
void printFont8(font8 f,int offset)
{
    ...
    for (i = 0; i < f.size; i++)
    {
        printFont8Letter(f.f[i],32+i,offset + i*8);
    }
}

font8 font8Editor(font8 in)
{
    font8 out = in;
    char action[512];
    while (strcmp(action,"exit"))
    {
        ...
        else if (!strcmp(action,"letters"))
        {
            ...
            for (i = 0; i < strlen(action);i++)
                printFont8Letter(out.f[action[i]-32],action[i],(action[i]-32)*8);
        }
        else if (!strcmp(action,"edit"))
        {
            ...
            if (!strcmp(characters,"\"\""))
            {
                ...
                printFont8Letter(out.f[' '-32],' ',(' '-32)*8);
            }
            ...
            else
            {
                ...
                for (k = 0; k < strlen(characters); k++)
                {
                    ...
                    printf("Successful edition. Your letter:\n");
                    printFont8Letter(out.f[editting-32],editting,(editting-32)*8);
                }
            }
        }
        else
